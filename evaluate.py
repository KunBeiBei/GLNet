...
def get_idcg(ground_truth, topn):
    if len(ground_truth) < topn:
        topn = len(ground_truth)
...
def get_ndcg(ranked_list, ground_truth, topn):
    if len(ground_truth) < topn:
        topn = len(ground_truth)
    dcg = 0
    idcg = get_idcg(ground_truth, topn)
    # print(ranked_list)
    # input()
    for i in range(topn):
        xid = ranked_list[i]
        dcg += ((2 ** ground_truth[xid]) - 1) / math.log(i+2, 2)
    if idcg == 0:
        return 1
    else:
        return dcg / idcg
